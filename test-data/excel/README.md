...
- 各类型必需表头：
  ...
  - 考勤：`学号, 姓名, 日期, 状态, 节次, 第几周, 备注`（状态使用中文：出勤/迟到/请假/缺勤）
  - 实验：`学号, 姓名, 实验名称, 实验次数, 分数, 提交时间, 备注`

### 2. 按表头列名匹配的类型（作业/测验/考试成绩）★ v3.1 更新

- **列顺序任意**，按表头文字匹配（如"总成绩"列不管在第几列都能识别）
- 表头示例：`序号 | 学号 | 姓名 | 第1题得分 | 扣分主要知识点 | ... | 第N题得分 | 扣分主要知识点 | 总成绩 | 最薄弱知识点`
- 题目数量由表头中的 `第X题得分` 自动检测
- 扣分知识点列识别规则：包含"扣分"和"知识点"即可（如"扣分知识点"、"扣分主要知识点"均可）
- **页面选课优先**：前端选择课程后传 `courseId` 参数，文件名仅作为没选课时的回退
