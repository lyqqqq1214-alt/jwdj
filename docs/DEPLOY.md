...
> 目标：答辩当天，任意一台电脑打开浏览器访问 `http://125.221.160.17:5150`，即可使用系统全部功能。
...
- 服务器：Windows 系统，公网 IP `125.221.160.17`，远程桌面端口 `32601`，账号 `summer` / `Summer@2026`，公网访问端口 `5150`（需路由器端口映射 + Windows 防火墙放行）。
...
在浏览器打开 `http://125.221.160.17:5150`：
...
- **端口 5150 打不开**：确认 Windows 防火墙已放行 TCP 5150，且路由器已做端口映射 `5150 → 内网IP:5150`。
...
- Ollama / MySQL / 后端 8080 都只监听 127.0.0.1，公网只暴露 Nginx 的 5150 端口。
